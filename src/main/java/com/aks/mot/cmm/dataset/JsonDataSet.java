...
public class JsonDataSet extends Constants {
	...
	public void setOutDs(String dsName, List list) {
		...
		if (list instanceof devonframe.paging.model.PagingList) {
			PagingList pagingList = (PagingList) list;
			int rows = pagingList.getRows();
			int rowSize = pagingList.getRowSize();
			int pageSize = pagingList.getPageSize();
			int currentPage = pagingList.getCurrentPage();
			Map<String, Object> pagingInfo = new HashMap<String, Object>();
			pagingInfo.put("rows", rows);
			pagingInfo.put("rowSize", rowSize);
			pagingInfo.put("pageSize", pageSize);
			pagingInfo.put("currentPage", currentPage);
			resBodyMap.put(dsName + "_paging", pagingInfo);
		}
	}
